...
def rank_momentum(monthly_universe,daily_returns,expiry_day,lookback_period=120):
    ...
    daily_rank=daily_returns_consideration_set.rank(axis=1)
    daily_rank=daily_rank.mean()
...
def creating_portfolio(cash_price_data,universe_stocks,trade_days,universal_dates,lookback_period=120):
    ...
    rank_momentum_universe=pd.DataFrame()
    ...
    for expiry_day in expiry_days_stocks.index:
        ...
        monthly_universe["Rank"]=rank_momentum(monthy_universe_list,daily_returns,expiry_day,lookback_period)

        buy_p=pd.DataFrame(monthly_universe["Rank"].astype(float).nlargest(10,keep="all").index)
        sell_p = pd.DataFrame(monthly_universe["Rank"].astype(float).nsmallest(10, keep="all").index)

        buy_portfolio=buy_portfolio.append(buy_p.T)
        sell_portfolio=sell_portfolio.append(sell_p.T)
        rank_momentum_universe=rank_momentum_universe.append(monthly_universe.T)
    ...
    rank_momentum_universe["expiry_days"]=expiry_days_stocks.index
    rank_momentum_universe.set_index("expiry_days",inplace=True)
    ...
    return buy_portfolio,sell_portfolio,monthly_universe_portfolios,rank_momentum_universe
...
if __name__=="__main__":
    ...
    buy_portfolio,sell_portfolio,monthly_universe_portfolios,rank_momentum_universe = creating_portfolio(cash_price_data, universe_stocks, expiry_days, universal_dates)

    buy_portfolio.name="Buy Portfolio"
    sell_portfolio.name="Sell Portfolio"
    rank_momentum_universe.name="Rank momentum returns"
    monthly_universe_portfolios.name="Monthly Universe"

    to_be_saved_as_file=[buy_portfolio,sell_portfolio,rank_momentum_universe,monthly_universe_portfolios]
    my_funcs.excel_creation(to_be_saved_as_file,"Portfolio Results/Rank Momentum","Rank_Momentum_Nifty")
