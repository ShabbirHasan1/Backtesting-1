...
def creating_portfolio(cash_price_data,universe_stocks,trade_days,universal_dates,period=90):

    cash_symbols_data_list=pd.Series(cash_price_data.keys())
    multiplier=math.sqrt(260)

    buy_portfolio=pd.DataFrame()
    sell_portfolio=pd.DataFrame()
    RSI_universe=pd.DataFrame()
    monthly_universe_portfolios=pd.DataFrame()
    daily_volatility=pd.DataFrame(0,columns=cash_symbols_data_list,index=universal_dates)
    ...
    for stock in cash_symbols_data_list:
        daily_returns[stock]=np.log(cash_price_data[stock].Close/cash_price_data[stock].Close.shift())
        daily_volatility[stock]=daily_returns[stock].rolling(period).std()*(multiplier)
    ...
    for expiry_day in expiry_days_stocks.index:
        ...
        rsi_numbers=daily_volatility.loc[expiry_day]
...
if __name__=="__main__":
    ...
    buy_portfolio,sell_portfolio,monthly_universe_portfolios,RSI_universe = creating_portfolio(cash_price_data, universe_stocks, expiry_days, universal_dates, period=90)
    ...
    my_funcs.excel_creation(to_be_saved_as_file,"Portfolio Results/D90_Vol","D90_volatility_Nifty")
