...
def creating_portfolio(cash_price_data,universe_stocks,trade_days,universal_dates):
    ...
    returns_12M=pd.DataFrame(0,columns=cash_symbols_data_list,index=universal_dates)
    returns_1M=pd.DataFrame(0,columns=cash_symbols_data_list,index=universal_dates)
    returns_12M_1M=pd.DataFrame(0,columns=cash_symbols_data_list,index=universal_dates)
    ...
    for stock in cash_symbols_data_list:
        ...
        returns_12M[stock]=(cash_price_data[stock].Close/cash_price_data[stock].Close.shift(250)-1)
        returns_1M[stock]=(cash_price_data[stock].Close/cash_price_data[stock].Close.shift(22)-1)

    returns_12M_1M=returns_12M-returns_1M
    ...
    for expiry_day in expiry_days_stocks.index:
        ...
        rsi_numbers=returns_12M_1M.loc[expiry_day]
        monthly_universe["RSI"]=rsi_numbers

        buy_p=pd.DataFrame(monthly_universe["RSI"].astype(float).nlargest(10,keep="all").index)
        sell_p = pd.DataFrame(monthly_universe["RSI"].astype(float).nsmallest(10, keep="all").index)
...
if __name__=="__main__":
    ...
    buy_portfolio,sell_portfolio,monthly_universe_portfolios,RSI_universe = creating_portfolio(cash_price_data, universe_stocks, expiry_days, universal_dates)

    buy_portfolio.name="Buy Portfolio"
    sell_portfolio.name="Sell Portfolio"
    RSI_universe.name="M12_M1 returns"
    ...
    my_funcs.excel_creation(to_be_saved_as_file,"Portfolio Results/M12_M1 returns","M12_M1_Nifty")
