...
def rsi2_system(price_data, period_rsi=2, period="", trade_type="Both_leg",
    ...
    price_signal_period["Signal"] = np.where(((price_signal_period["rsi"] > 10) & (price_signal_period["rsi_1"] <10)),1, np.where(
        ((price_signal_period["rsi"] < 90) & (price_signal_period["rsi_1"] > 90)),-1,0))

    price_signal_period["Signal"].replace(to_replace=0,method="ffill",inplace=True)
