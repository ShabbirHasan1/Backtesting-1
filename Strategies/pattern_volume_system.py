...
def pattern_volume_system(price_data, atr_period=44, daily_range_period=4,period="", trade_type="Both_leg", underlying_instrument_data=None):
    if period == "":
    ...
    Volume_min=price_signal_period["Volume"].rolling(4).min()

    price_signal_period["Trading Window"] = np.where((price_signal_period["4 DR"] < price_signal_period["ATR"])
                                                     & (price_signal_period["Volume"] == Volume_min), 1, 0)
